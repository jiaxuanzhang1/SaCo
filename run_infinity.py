...
def encode_prompt(text_tokenizer, text_encoder, prompt, enable_positive_prompt=False, target_word=None):
    ...
    if target_word:
        ...
        target_token = text_tokenizer(target_word, add_special_tokens=False)["input_ids"]
        
        if not target_token:
            pass
        else:
            input_tokens = input_ids[0].tolist()
            target_token_list = target_token
            token_length = len(target_token_list)

            valid_length = mask[0].sum().item()
            
            for i in range(0, valid_length - token_length + 1):

                if input_tokens[i:i+token_length] == target_token_list:
                    target_idx = i
                    # print(f"Found target word '{target_word}' at position {i}")
                    break
    ...
    if target_word:
        return text_cond_tuple, target_idx
...
def infer4code(
    ...
    if target_word is not None:
        text_cond_tuple, target_idx = encode_prompt(text_tokenizer, text_encoder, prompt, enable_positive_prompt, target_word=target_word)
    else:
        text_cond_tuple = encode_prompt(text_tokenizer, text_encoder, prompt, enable_positive_prompt)
        target_idx = None
    # text_cond_tuple = encode_prompt(text_tokenizer, text_encoder, prompt, enable_positive_prompt, P=debias)
    pos_text_cond_tuple = encode_prompt(text_tokenizer, text_encoder, pos_prompt, enable_positive_prompt)
    if negative_prompt:
    ...
    with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        stt = time.time()
        _, idx_Bl_list, img_list, sen_bld_list, to_pos_bld_list = infinity_test.infer4code(
            vae=vae,
            scale_schedule=scale_schedule,
            label_B_or_BLT=text_cond_tuple, pos_label_B_or_BLT=pos_text_cond_tuple, g_seed=g_seed,
            ...
            target_idx=target_idx, top_attn=top_attn, ssi=ssi, esi=esi,
        )
    # print(f"cost: {time.time() - sstt}, infinity cost={time.time() - stt}")
    img = img_list[0]
    return img, idx_Bl_list, sen_bld_list, to_pos_bld_list
...
def gen_one_img(
    ...
    if sld_para is not None:
        sld_safe_concept, sld_guidance_scale, sld_warmup_steps, sld_threshold, sld_momentum_scale, sld_mom_beta = sld_para
        print(sld_safe_concept)
        safe_concept_label_B_or_BLT = encode_prompt(text_tokenizer, text_encoder, sld_safe_concept)
        sld_para = (safe_concept_label_B_or_BLT, sld_guidance_scale, sld_warmup_steps, sld_threshold, sld_momentum_scale, sld_mom_beta)
    else:
        sld_para = None
    print(f'cfg: {cfg_list}, tau: {tau_list}')
    with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        ...
        _, _, img_list = infinity_test.autoregressive_infer_cfg(
            ...
            gt_leak=gt_leak, gt_ls_Bl=gt_ls_Bl, inference_mode=True,
            sampling_per_bits=sampling_per_bits,
            code_path=code_path, alpha=alpha, ssi=ssi, esi=esi, replace_code=replace_code,
