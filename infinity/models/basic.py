...
class CrossAttention(nn.Module):
    def __init__(
        ...
        proj_drop=0., cos_attn=False, use_flash=True
    ):
        """
        ...
        :param cos_attn: during attention, q and k will be L2-normalized and scaled
        :param use_flash: whether to use Flash Attention (faster) or standard Attention (can return attn map)
        """
        cos_attn = False  # Currently disabled
        super().__init__()
    ...
    def forward(self, q, ca_kv, return_attn_map=False, target_idx=None):
        """
        :param q: shaped as (batch, seq_len, Q_dim)
        :param ca_kv: contains several vectors, each of which is shaped as (len_i, KV_dim). 
            - kv_compact: shaped as (sum(lens), KV_dim)
            - cu_seqlens_k: cumulated sum of lens
            - max_seqlen_k: int, max(lens)
        :param return_attn_map: whether to return attention map (only supported in standard mode)
        :return: 
            - output: shaped as (batch, seq_len, Q_dim)
            - attn_map: attention weights if return_attn_map=True and use_flash=False
        """
        kv_compact, cu_seqlens_k, max_seqlen_k = ca_kv
        N = kv_compact.shape[0]
        B = cu_seqlens_k.shape[0] - 1  # Number of batches
        
        # Prepare KV pairs
        kv_compact = F.linear(kv_compact, 
                              weight=self.mat_kv.weight, 
                              bias=torch.cat((self.zero_k_bias, self.v_bias)))
        kv_compact = kv_compact.view(N, 2, self.num_heads, self.head_dim)
        k, v = kv_compact.unbind(1)  # k/v shape: (N, num_heads, head_dim)

        # Prepare Q
        if not self.for_attn_pool:
            Lq = q.shape[1]
            q_compact = self.mat_q(q)  # (B, Lq, embed_dim)
            q_compact = q_compact.view(B, Lq, self.num_heads, self.head_dim)
        else:
            Lq = 1
            q_compact = self.mat_q.repeat(B, 1, 1).to(dtype=kv_compact.dtype)  # (B, num_heads, head_dim)
            q_compact = q_compact.unsqueeze(1)  # (B, 1, num_heads, head_dim)
        
        # Optional cosine attention (currently disabled)
        if self.cos_attn:
        ...
        # ========================================================================
        # Select attention implementation based on use_flash
        # ========================================================================
        if return_attn_map == False:
            # Flash Attention implementation (faster but can't return attn map)
            q_compact = q_compact.reshape(-1, self.num_heads, self.head_dim)  # (B*Lq, num_heads, head_dim)
            kv_compact = torch.stack([k, v], dim=1)  # Recreate kv_compact
            
            # Use Flash Attention
            from flash_attn import flash_attn_varlen_kvpacked_func  # Import here to avoid dependency if not used
            ...
            output = self.proj_drop(self.proj(oup))
            
            # Flash attention doesn't support returning attention map
            if return_attn_map:
                raise NotImplementedError("Attention map cannot be returned when using Flash Attention")
            return output
        else:
            # Standard Attention implementation (slower but can return attn map)
            # Split kv sequences per batch
            seq_lens_k = cu_seqlens_k[1:] - cu_seqlens_k[:-1]
            k_list = torch.split(k, seq_lens_k.tolist(), dim=0)
            v_list = torch.split(v, seq_lens_k.tolist(), dim=0)
            
            # Pad sequences to max_seqlen_k
            k_padded = torch.zeros(B, max_seqlen_k, self.num_heads, self.head_dim, 
                                  device=k.device, dtype=k.dtype)
            v_padded = torch.zeros(B, max_seqlen_k, self.num_heads, self.head_dim, 
                                  device=v.device, dtype=v.dtype)
            mask = torch.zeros(B, max_seqlen_k, device=k.device, dtype=torch.bool)
            
            for i, (k_i, v_i) in enumerate(zip(k_list, v_list)):
                Lk = k_i.shape[0]
                k_padded[i, :Lk] = k_i
                v_padded[i, :Lk] = v_i
                mask[i, Lk:] = True  # Mark padded positions
            
            # Convert mask to attention format
            mask = mask.unsqueeze(1).unsqueeze(1)  # (B, 1, 1, max_seqlen_k)
            
            # Rearrange dimensions for attention calculation
            # Q: (B, Lq, num_heads, head_dim) -> (B, num_heads, Lq, head_dim)
            # K: (B, max_seqlen_k, num_heads, head_dim) -> (B, num_heads, head_dim, max_seqlen_k)
            # V: (B, max_seqlen_k, num_heads, head_dim) -> (B, num_heads, max_seqlen_k, head_dim)
            q_trans = q_compact.permute(0, 2, 1, 3)
            k_trans = k_padded.permute(0, 2, 3, 1)
            v_trans = v_padded.permute(0, 2, 1, 3)
            
            # Compute attention scores
            attn_scores = torch.matmul(q_trans, k_trans) * self.scale  # (B, num_heads, Lq, max_seqlen_k)
            ...
            attn_output = torch.matmul(attn_weights, v_trans)  # (B, num_heads, Lq, head_dim)
            
            # Revert to original shape (B, Lq, embed_dim)
            attn_output = attn_output.permute(0, 2, 1, 3)  # (B, Lq, num_heads, head_dim)
            output = attn_output.reshape(B, Lq, self.embed_dim)
            
            # Final projection and dropout
            output = self.proj_drop(self.proj(output))
            # print('attn score', attn_weights[0, :, :, target_idx].shape)
            # Return attention maps if requested
            if return_attn_map:
                return output, torch.mean(attn_scores[0, :, :, target_idx], dim=0)  # attn_weights
            else:
                return output
    
    def extra_repr(self) -> str:
        return f'Cq={self.embed_dim}, Ckv={self.kv_dim}, cos_attn={self.cos_attn}, use_flash={self.use_flash}'
...
class SelfAttnBlock(nn.Module):
    def __init__(
        ...
        swiglu=False, customized_flash_attn=False, fused_mlp=False, fused_norm_func=None, checkpointing_sa_only=False, attn_fn=None
    ):
...
class CrossAttnBlock(nn.Module):
    ...
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0, return_attn_map=False, target_idx=None, use_flash=True):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_norm_func is None:
            ...
            if return_attn_map:
                res, attn_map = self.ca(self.ca_norm(x), ca_kv, return_attn_map, target_idx)
                x = x + res.float().mul_(self.ca_gamma)
            else:
