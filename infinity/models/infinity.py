...
class Infinity(nn.Module):
    def __init__(
        ...
        # self.lookup = torch.tensor([bin(x).count('1') for x in range(256)], 
        #                     dtype=torch.uint8, device='cuda:1')
    ...
    def autoregressive_infer_cfg(
        ...
        if sld_para is not None:
            safe_concept_label_B_or_BLT, sld_guidance_scale, sld_warmup_steps, sld_threshold, sld_momentum_scale, sld_mom_beta = sld_para
        ...
        if any(np.array(cfg_list) != 1):
            ...
            if sld_para is not None:
                bs =3*B
                kv_compact_safe, len_safe, cu_seqlens_k_safe, max_seqlen_k_safe = safe_concept_label_B_or_BLT
                kv_compact = torch.cat((kv_compact, kv_compact_safe), dim=0)
                cu_seqlens_k = torch.cat((cu_seqlens_k, cu_seqlens_k_safe[1:]+cu_seqlens_k[-1]), dim=0)
                max_seqlen_k = max(max_seqlen_k, max_seqlen_k_safe)
        else:
            bs = B
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            # print("scale:",si,'; laststage shape:',last_stage.shape)
            ...
            for block_idx, b in enumerate(self.block_chunks):  # 4
            ...
            if (cfg != 1) and add_cfg_on_logits:
                ...
                if not sld_para:
                    ...
                    logits_BlV = self.get_logits(last_stage, cond_BD).mul(1/tau_list[si])
                    logits_Blv_text, logits_BlV_un, logits_BlV_safe = logits_BlV[:B], logits_BlV[B:2*B], logits_BlV[2*B:]
                    # print("logits text shape:", logits_Blv_text.shape)


                    logits_BlV_guidance = (logits_Blv_text - logits_BlV_un)

                    if safety_momentum is None:
                        safety_momentum = torch.zeros_like(logits_BlV_guidance)

                    scale = torch.clamp(torch.abs((logits_Blv_text - logits_BlV_safe)) * sld_guidance_scale, max=1.)
                    safety_concept_scale = torch.where(
                        (logits_Blv_text - logits_BlV_safe) >= sld_threshold,
                        torch.zeros_like(scale), scale)
                    
                    logits_BlV_guidance_safe = torch.mul(
                        (logits_BlV_safe - logits_BlV_un), safety_concept_scale)

                    # Equation 7
                    logits_BlV_guidance_safe = logits_BlV_guidance_safe + sld_momentum_scale * safety_momentum

                    # Equation 8
                    safety_momentum = sld_mom_beta * safety_momentum + (1 - sld_mom_beta) * logits_BlV_guidance_safe

                    if si >= sld_warmup_steps: # Warmup
                        # Equation 3
                        logits_BlV_guidance = logits_BlV_guidance - logits_BlV_guidance_safe


                    logits_BlV = logits_BlV_un + cfg * logits_BlV_guidance
            else:
            ...
            if vae_type != 0:
                ...
                # ------------------------------------------select index----------------------------------------------
                h, w = idx_Bld.shape[-3], idx_Bld.shape[-2]
                
                # idx_Bld: [bs, t=1, h, w, d=32]
                if code_path is not None and si < esi and si >= ssi:
                    # print('idx shape:', idx_Bld.shape)
    
                    sen_code = torch.load(os.path.join(code_path, "scale" + str(si) + "_sen.pt")).to('cuda')
                    pos_code = torch.load(os.path.join(code_path, "scale" + str(si) + "_pos.pt")).to('cuda')
                    # print(type(fl_tensors))
                    # print(fl_tensors)



                    if sen_code.shape[0] > 0:
                       
                        mask, min_dist, ind_in_negbook = anchor_neg_codes(neg_book=sen_code, now_codes=idx_Bld[0, 0].clone().detach(), threshold=alpha)

                        num_neg_codes = mask.sum().item()

                        if rand_mask:
                            random_mask = torch.zeros_like(mask)
                            rand_idx = torch.randperm(mask.numel())[:int(num_neg_codes)]
                        ...
                        if replace_code:
                            if rand_mask:
                                idx_Bld[0, 0].view(-1, 32)[random_mask] = pos_code[ind_in_negbook[mask]].to(idx_Bld.device)
                            else:
                                idx_Bld[0, 0].view(-1, 32)[mask] = pos_code[ind_in_negbook[mask]].to(idx_Bld.device)
                        else:
                            if rand_mask:
                                idx_Bld[0, 0] = fill_neg_codes(idx_Bld[0, 0], random_mask.view(h, w))
                            else:
                                idx_Bld[0, 0] = fill_neg_codes(idx_Bld[0, 0], mask.view(h, w))
                        idx_Bld[0, 0] = idx_Bld[0,0].view(h,w,32)
            ...
            if si != num_stages_minus_1:
                ...
                if sld_para:
                    safety_momentum = expand_tensor(safety_momentum, vae_scale_schedule[si][-1], vae_scale_schedule[si+1][-1])
        ...
        if vae_type != 0:
            # print(summed_codes.squeeze(-3).shape)
            img = vae.decode(summed_codes.squeeze(-3))
            # img = vae.decode(slice_code.squeeze(-3))
    ...
    def autoregressive_infer_sld(
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            if (cfg != 1) and add_cfg_on_logits:
                # print(f'add cfg on add_cfg_on_logits')
                if not safe_concept_label_B_or_BLT:
                    logits_BlV = self.get_logits(last_stage, cond_BD).mul(1/tau_list[si])
                    logits_BlV = cfg * logits_BlV[:B] + (1-cfg) * logits_BlV[B:]
                else:
                    logits_BlV = self.get_logits(last_stage, cond_BD).mul(1/tau_list[si])
            ...
            if vae_type != 0:
                ...
                if code_path is not None and si < esi and si >= ssi:
                    ...
                    if sen_code.shape[0] > 0:
                        mask, min_dist, ind = anchor_neg_codes(sen_code, idx_Bld[0, 0].clone().detach(), threshold=alpha, si=si)
                        true_count = mask.sum().item()

                        
                        if true_count > 0: 
                            if scale_con_map is None:
                                scale_con_map = mask.reshape(h, w)
                            else:
                                mask = mask.reshape(h, w)
                                scale_con_map = resize_2D_01_mask_tensor(scale_con_map, h)
                                mask = scale_con_map & mask
                                mask = mask.reshape(h*w, )
                        else:
                            if scale_con_map is None:
                                pass
                            else:
                                scale_con_map = torch.zeros(h,w).to('cuda')


                        idx_Bld[0, 0].view(-1, 32)[mask] = pos_code[ind[mask]].to(idx_Bld.device)   # int_to_binary(flt_pos_tensors[ind[mask]]).to(idx_Bld.device)
                        idx_Bld[0, 0] = idx_Bld[0,0].view(h,w,32)
            ...
            if si != num_stages_minus_1:
                last_stage = self.word_embed(self.norm0_ve(last_stage))
                last_stage = last_stage.repeat(bs//B, 1, 1)
                if safe_concept_label_B_or_BLT:
                    safety_momentum = expand_tensor(safety_momentum, vae_scale_schedule[si][-1], vae_scale_schedule[si+1][-1])
    ...
    def forward4evar(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        cfg_infer=False, device='cuda',
        **kwargs,
    ) -> Union[torch.Tensor, List[torch.Tensor]]:  # returns logits_BLV
        """
        label_B_or_BLT: label_B or (kv_compact, cu_seqlens_k, max_seqlen_k)
        ...
        # with torch.amp.autocast('cuda', enabled=False):
        kv_compact, lens, cu_seqlens_k, max_seqlen_k = label_B_or_BLT
        kv_compact = kv_compact.to(device)
        ...
        sos = sos.unsqueeze(1).expand(B, 1, -1) + self.pos_start.expand(B, 1, -1)
        # x_BLC = torch.cat((sos, self.word_embed(self.norm0_ve(x_BLC_wo_prefix))), dim=1)
        x_BLC = torch.cat((sos, x_BLC_wo_prefix), dim=1)
    ...
    def infer4code(
        ...
        kv_compact, lens, cu_seqlens_k, max_seqlen_k = label_B_or_BLT
        pos_kv_compact, pos_lens, pos_cu_seqlens_k, pos_max_seqlen_k = pos_label_B_or_BLT
        ...
        if any(np.array(cfg_list) != 1):
            ...
            if not negative_label_B_or_BLT:
                ...
                # pos
                pos_kv_compact_un = pos_kv_compact.clone()
                total = 0
                for le in pos_lens:
                    pos_kv_compact_un[total:total+le] = (self.cfg_uncond)[:le]
                    total += le
                pos_kv_compact = torch.cat((pos_kv_compact, pos_kv_compact_un), dim=0)
                pos_cu_seqlens_k = torch.cat((pos_cu_seqlens_k, pos_cu_seqlens_k[1:]+pos_cu_seqlens_k[-1]), dim=0)
        ...
        pos_kv_compact = self.text_norm(pos_kv_compact)
        pos_sos = pos_cond_BD = self.text_proj_for_sos((pos_kv_compact, pos_cu_seqlens_k, pos_max_seqlen_k)) # sos shape: [2, 4096]
        pos_kv_compact = self.text_proj_for_ca(pos_kv_compact) # kv_compact shape: [304, 4096]
        pos_ca_kv = pos_kv_compact, pos_cu_seqlens_k, pos_max_seqlen_k
        pos_last_stage = pos_sos.unsqueeze(1).expand(bs, 1, -1) + self.pos_start.expand(bs, 1, -1)

        with torch.amp.autocast('cuda', enabled=False):
            cond_BD_or_gss = self.shared_ada_lin(cond_BD.float()).float().contiguous()
            pos_cond_BD_or_gss = self.shared_ada_lin(pos_cond_BD.float()).float().contiguous()
        accu_BChw, cur_L, ret = None, 0, []  # current length, list of reconstructed images
        idx_Bl_list, idx_Bld_list = [], []
        sen_code_list = [None for _ in range(ssi, esi)]
        pos_code_list = [None for _ in range(ssi, esi)]
        ...
        num_stages_minus_1 = esi - 1
        summed_codes = 0
        attn_map_list = []
        # [(1, 1, 1), (1, 2, 2), (1, 4, 4), (1, 6, 6), (1, 8, 8), (1, 12, 12), (1, 16, 16), (1, 20, 20), (1, 24, 24), (1, 32, 32), (1, 40, 40), (1, 48, 48), (1, 64, 64)]
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            if si >= esi:
                continue
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                for m in b.module:
                    last_stage, attn_map = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si, return_attn_map=True, target_idx=target_idx)
                    if attn_map_sum is None:
                        attn_map_sum = attn_map
                    else:
                        attn_map_sum += attn_map  # scale_dim, 1
            ...
            attn_map_list.append(attn_map_sum)
        ...
        last_stage = pos_last_stage





        final_attn_map = attn_map_list[-1]
        print("final attn:", final_attn_map.shape)
        top_attn = 0.01 * top_attn
        final_h = scale_schedule[esi - 1][1]
        final_w = scale_schedule[esi - 1][2]

        _, final_attn_topk_indices = torch.topk(final_attn_map, k=int(top_attn * final_h * final_w), largest=True)
        final_attn_mask = torch.zeros_like(final_attn_map, dtype=torch.bool, device=final_attn_map.device)
    
        final_flat_mask = final_attn_mask.flatten()
        final_flat_mask[final_attn_topk_indices] = True

        final_attn_mask = final_flat_mask.reshape((final_h,final_w))
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                for m in b.module:
                    pos_last_stage = m(x=last_stage, cond_BD=pos_cond_BD_or_gss, ca_kv=pos_ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si)
                    if (cfg != 1) and (layer_idx in abs_cfg_insertion_layers):
                        pos_last_stage = cfg * pos_last_stage[:B] + (1-cfg) * pos_last_stage[B:]
                        pos_last_stage = torch.cat((pos_last_stage, pos_last_stage), 0)
                    layer_idx += 1
            
            if (cfg != 1) and add_cfg_on_logits:

                pos_logits_BlV = self.get_logits(pos_last_stage, pos_cond_BD).mul(1/tau_list[si])
                pos_logits_BlV = cfg * pos_logits_BlV[:B] + (1-cfg) * pos_logits_BlV[B:]
            else:

                pos_logits_BlV = self.get_logits(pos_last_stage[:B], pos_cond_BD[:B]).mul(1/tau_list[si])
            
            if self.use_bit_label:
               
                pos_tmp_bs, pos_tmp_seq_len = pos_logits_BlV.shape[:2]
                pos_logits_BlV = pos_logits_BlV.reshape(pos_tmp_bs, -1, 2)
                pos_idx_Bld = sample_with_top_k_top_p_also_inplace_modifying_logits_(pos_logits_BlV, rng=rng, top_k=top_k or self.top_k, top_p=top_p or self.top_p, num_samples=1)[:, :, 0]
                pos_idx_Bld = pos_idx_Bld.reshape(pos_tmp_bs, pos_tmp_seq_len, -1)
            else:
                idx_Bl = sample_with_top_k_top_p_also_inplace_modifying_logits_(logits_BlV, rng=rng, top_k=top_k or self.top_k, top_p=top_p or self.top_p, num_samples=1)[:, :, 0]
            if vae_type != 0:
                assert returns_vemb
                if si < gt_leak:
                    ...
                    pos_idx_Bld = gt_ls_Bl[si]
                else:
                    assert pn[0] == 1

                    pos_idx_Bld = pos_idx_Bld.reshape(B, pn[1], pn[2], -1) # shape: [B, h, w, d] or [B, h, w, 4d]
                    if self.apply_spatial_patchify: # unpatchify operation
                        pos_idx_Bld = pos_idx_Bld.permute(0,3,1,2) # [B, 4d, h, w]
                        pos_idx_Bld = torch.nn.functional.pixel_shuffle(pos_idx_Bld, 2) # [B, d, 2h, 2w]
                        pos_idx_Bld = pos_idx_Bld.permute(0,2,3,1) # [B, 2h, 2w, d]
                    pos_idx_Bld = pos_idx_Bld.unsqueeze(1) # [B, 1, h, w, d] or [B, 1, 2h, 2w, d]

                

                # ------------------------------------------select index----------------------------------------------
                if si < esi and si >= ssi:
                    pos_idx = pos_idx_Bld
                    idx_Bld = idx_Bld_list[si]
                    attn_map = attn_map_list[si]

                    h, w = idx_Bld.shape[-3], idx_Bld.shape[-2]



                    attn_k = int(top_attn * h * w)
                    # print('attn map', attn_map.unsqueeze(-1).shape)
                    # print('len', attn_k)
                    _, attn_topk_indices = torch.topk(attn_map, k=attn_k, largest=True)

                    

                    _, attn_topk_indices = torch.topk(attn_map, k=attn_k, largest=True)

                    condition_mask = resize_2D_01_mask_tensor(final_attn_mask, h)
                    flat_condition_mask = condition_mask.flatten()


                    mask_at_indices = flat_condition_mask[attn_topk_indices]
                    filtered_indices = attn_topk_indices[mask_at_indices]


                    if filtered_indices.numel() > 0:
                        common_indices = filtered_indices

                        h_indices = common_indices // w
                        w_indices = common_indices % w


                        sen_code_list[si - ssi] = idx_Bld[0, 0, h_indices, w_indices, :].clone().detach().view(-1, 32)
                        pos_code_list[si - ssi] = pos_idx[0, 0, h_indices, w_indices, :].clone().detach().view(-1, 32)


                # ----------------------------------------------------------------------------------------------------
                

                codes = vae.quantizer.lfq.indices_to_codes(pos_idx_Bld, label_type='bit_label') # [B, d, 1, h, w] or [B, d, 1, 2h, 2w]
        ...
        return ret, idx_Bld_list, img, sen_code_list, pos_code_list
    ...
    def infer4evar(
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            if vae_type != 0:
                ...
                if si != num_stages_minus_1:
                    # if si == deadsi:
                    #     slice_code = F.interpolate(codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
                    summed_codes += F.interpolate(codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
            ...
            if si != num_stages_minus_1:
                last_stage = self.word_embed(self.norm0_ve(last_stage))
                x_wo_prefix_BLC.append(last_stage)
                last_stage = last_stage.repeat(bs//B, 1, 1)
        ...
        x_wo_prefix_BLC = torch.cat(x_wo_prefix_BLC, dim=1)
        # print(x_wo_prefix_BLC.shape)
        if return_img:
            if vae_type != 0:
                # print(summed_codes.squeeze(-3).shape)
                img = vae.decode(summed_codes.squeeze(-3))
            ...
            img = (img + 1) / 2
            img = img.permute(0, 2, 3, 1).mul_(255).to(torch.uint8).flip(dims=(3,))
            return x_wo_prefix_BLC, img

        return x_wo_prefix_BLC
...
def expand_tensor(tensor, original_size, new_size):

    bs, _, length = tensor.shape
    
    tensor_reshaped = tensor.view(bs, original_size, original_size, length)
    
    tensor_permuted = tensor_reshaped.permute(0, 3, 1, 2)

    tensor_upsampled = F.interpolate(
        tensor_permuted, 
        size=(new_size, new_size), 
        mode='nearest'
    )
    
    tensor_restored = tensor_upsampled.permute(0, 2, 3, 1)

    result = tensor_restored.contiguous().view(bs, new_size * new_size, length)
    
    return result

def fill_neg_codes(tensor, mask):
    ...
    coords = torch.stack([y_coords, x_coords], dim=-1).float().to(device=device)  # [h, w, 2]
    

    source_coords = coords[mask == 0]  # [num_source, 2]
    source_vectors = tensor[mask == 0]  # [num_source, 32]
    

    target_coords = coords[mask == 1]  # [num_target, 2]
    
    if source_coords.size(0) == 0 or target_coords.size(0) == 0:
        return tensor
    
    dists = torch.cdist(target_coords.unsqueeze(0), source_coords.unsqueeze(0)).squeeze(0)
    
    _, nn_indices = torch.min(dists, dim=1)
    
    nearest_vectors = source_vectors[nn_indices]

    result = tensor.clone()
    
    result[mask == 1] = nearest_vectors
    
    return result

def resize_2D_01_mask_tensor(mask_tensor, new_size):

    n = mask_tensor.shape[0]
    if n == new_size:
        return mask_tensor.clone()
    
    input_4d = mask_tensor.view(1, 1, n, n).float()
    
    resized = F.interpolate(input_4d, 
                           size=new_size, 
                           mode='nearest', 
                           recompute_scale_factor=False)
    
    return resized.squeeze().byte()
...
def save_full_clustering_results(results, filepath):

    with open(filepath, 'wb') as f:
        pickle.dump(results, f)

def anchor_neg_codes_bf16(neg_book, now_codes, threshold=3):

    device = now_codes.device
    h, w, _ = now_codes.shape
    k = neg_book.shape[0]

    dtype = torch.bfloat16  
    neg_book = neg_book.to(device=device, dtype=dtype)
    now_codes = now_codes.to(device=device, dtype=dtype)

    now_codes_flat = now_codes.reshape(h * w, 32)  # (N, 32)
    N = now_codes_flat.shape[0]

    neg_sum_all = neg_book.sum(dim=1)              # (k,), dtype


    min_dist = torch.full((N,), float("inf"), device=device, dtype=torch.float32)
    ind_in_negbook = torch.zeros(N, device=device, dtype=torch.long)
    ...
    for n_start in range(0, N, n_chunk_size):
        ...
        chunk = now_codes_flat[n_start:n_end]      # (C, 32)
        C = chunk.shape[0]

        best_dist = torch.full((C,), float("inf"), device=device, dtype=torch.float32)
        best_idx = torch.zeros(C, device=device, dtype=torch.long)

        now_sum = chunk.sum(dim=1, keepdim=True)   # (C,1), dtype

        for k_start in range(0, k, k_chunk_size):
            k_end = min(k_start + k_chunk_size, k)

            neg_chunk = neg_book[k_start:k_end]            # (Kc, 32)
            neg_sum = neg_sum_all[k_start:k_end]           # (Kc,)

            dot_sub = chunk @ neg_chunk.t()                # (C, Kc), dtype

            dist_sub = dot_sub                             # alias
            dist_sub.mul_(-2)                              # = -2 * dot
            dist_sub.add_(now_sum)                         # + now_sum (C,1)
            dist_sub.add_(neg_sum.unsqueeze(0))            # + neg_sum (1,Kc)
            dist_sub.clamp_min_(0)

            sub_min, sub_idx = dist_sub.min(dim=1)         # (C,), (C,)
            sub_min = sub_min.to(torch.float32)

            better = sub_min < best_dist
            best_dist[better] = sub_min[better]
            best_idx[better] = sub_idx[better] + k_start


            del dot_sub, dist_sub, sub_min, sub_idx, neg_chunk, neg_sum


        min_dist[n_start:n_end] = best_dist
        ind_in_negbook[n_start:n_end] = best_idx

        del chunk, now_sum, best_dist, best_idx

    thr = torch.tensor(threshold, device=device, dtype=min_dist.dtype)
    mask = min_dist < thr

    return mask, min_dist, ind_in_negbook


def anchor_neg_codes(neg_book, now_codes, threshold=3):
    """
    Args:
        neg_book: Tensor of shape (k, 32)
        now_codes: Tensor of shape (h, w, 32)
        threshold: Distance threshold (default=3)
    Returns:
        Modified B where certain positions are set to 1
    """
    neg_book = neg_book.to(now_codes.device)
    h, w, _ = now_codes.shape
    neg_book = neg_book.float()
    now_codes = now_codes.float()
    now_codes_flat = now_codes.reshape(h * w, 32)  # Flatten to (h*w, 32) for easier computation

    # Compute pairwise distances between A and now_codes_flat
    # dist[i, j] = distance(B_flat[i], A[j])
    dist = torch.cdist(now_codes_flat, neg_book, p=2)**2  # shape (h*w, k)

    # Find the minimum distance for each B_flat[i]
    min_dist, ind_in_negbook = torch.min(dist, dim=1)  # shape (h*w,)

    mean_dist = torch.mean(dist, dim=1)

    # Create a mask where min_dist < threshold
    mask = min_dist < threshold  # shape (h*w,)
