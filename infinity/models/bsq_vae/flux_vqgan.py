...
class AutoEncoder(nn.Module):
        ...
        if not args.use_vae:
            if args.quantizer_type == 'MultiScaleBSQ':
                self.quantizer = MultiScaleBSQ(
                    ...
                    diversity_gamma = args.diversity_gamma,         # within entropy loss, how much weight to give to diversity of codes
    ...
    def forward(self, x):
        ...
        print('after encoder size:', h.shape)
        # print(z.shape)
        # Multiscale LFQ            
        z, all_indices, _, _, all_loss, _ = self.quantizer(h)
        print('after quantizer size:', z.shape)
